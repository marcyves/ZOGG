...
{
    global $websiteName, $websiteDescription, $template, $mysqli, $emailActivation, $user, $INX_SIDE1, $INX_LINKS;
    global $errors;
    global $successes;
    ...
    if($user->isLoggedIn())
    {
      ...
      $text = $user->data()->id." (<i>$userLevel</i>)";
    ...
    if (isUserReady($user->data()->id))
     {
      	if (hasPerm([1],$user->data()->id))
        { //Links for permission level 1 (user)
 		    }
		    if (hasPerm([2],$user->data()->id))
        { //Links for permission level 2 (admin)
          toolBarLink('management_campus.php', 'Campus and Courses Management','fa-wrench');
        ...
        if (hasPerm([3],$user->data()->id))
        { //Links for permission level 3 (professor)
        ...
        { //Links for permission level 4 (student)
